...
{
    public partial class RulesReact : LoginPage
    {
            ...
            if (!IsPostBack)
            {
                switch (helper.GetParam("action").ToLower())
                {
                    case "getlist":
                        ...
                        {
                            items = db.findPage<Wlniao.WeChat.Model.Rules>("RuleType=0 ", pageIndex, pageSize);
                        }
                        else
                        {
                            items = db.findPage<Wlniao.WeChat.Model.Rules>("RuleType=0 and (AccountFirst='' or AccountFirst='" + helper.GetParam("firstId") + "')", pageIndex, pageSize);
                        }
