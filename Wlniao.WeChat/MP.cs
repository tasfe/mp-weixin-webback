...
namespace Wlniao.WeChat.WeixinMP
{
	public class MP
	{
        ...
        public static Msg Init(string appid, string secret, out string token)
        {
            if (err.Count == 0)
            {
                err.Add("", "");
        ...
        public static string SyncMenu(string appid, string secret, string json)
		{
			try {
                string token = "";
                Msg msg = Init(appid, secret, out token);
				...
				msg = Json.ToObject<Msg> (result);
                if (msg != null && msg.errcode == "0")
                {
					return "菜单更新成功";
				}else{
                    return "错误：" + err[msg.errcode].ToString();
				}
			} catch(Exception ex) {
