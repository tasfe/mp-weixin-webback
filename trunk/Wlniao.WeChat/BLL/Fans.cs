...
namespace Wlniao.WeChat.BLL
{
    public class Fans : System.ORM.CommonBase<Model.Fans>
	{
        ...
        /// <returns></returns>
        public static Model.Fans Check(string clientUser,string serverUser, string text)
        {
            Model.Fans fans = GetBy("WeChatOpenId", clientUser);
            if (fans == null)
            {
                fans = new Model.Fans();
                fans.Guid = Guid.NewGuid().ToString();
                fans.WeChatOpenId = clientUser;
                fans.AccountFirst = serverUser;
                fans.Subscribe = 1;
        ...
        /// <param name="openid"></param>
        /// <returns></returns>
        public static Model.Fans SetSession(string clientUser, string KeyWords)
        {
            Model.Fans fans = GetBy("WeChatOpenId", clientUser);
            if (fans != null)
            {
                fans.KeyWords = KeyWords;
                fans.LastVisit = DateTools.GetNow();
                fans.LastCmdTime = DateTools.GetNow();
                fans.update(new string[] { "KeyWords", "LastVisit", "LastCmdTime" });
            }
            return fans;
        }
        /// <summary>
