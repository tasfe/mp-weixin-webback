...
{
    public partial class RulesMPApi : LoginPage
    {
            ...
            if (!IsPostBack)
            {
                switch (helper.GetParam("action").ToLower())
                {
                    case "getlist":
                        ...
                        {
                            items = db.findPage<Wlniao.WeChat.Model.Rules>("RuleType=3 ", pageIndex, pageSize);
                        }
                        else
                        {
                            items = db.findPage<Wlniao.WeChat.Model.Rules>("RuleType=3 and (AccountFirst='' or AccountFirst='" + helper.GetParam("firstId") + "')", pageIndex, pageSize);
                        }
