...
namespace WeChat.Base
{
	public partial class MyButton : LoginPage
	{
		...
		protected string Sync ()
		{
            ...
            foreach (Wlniao.WeChat.Model.WeiXin item in items)
            {
                try
                {
                    if (!string.IsNullOrEmpty(item.Secret) && !string.IsNullOrEmpty(item.Appid))
                    {
                        Wlniao.WeChat.WeixinMP.MP.SyncMenu(item.Appid, item.Secret, json);
                    }
                }
                catch { }
            }
