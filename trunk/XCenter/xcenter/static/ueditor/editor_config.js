...
(function () {
    ...
    var URL = window.UEDITOR_HOME_URL || '../../xcenter/static/ueditor/';
//    var tmp = window.location.pathname,
//            URL = window.UEDITOR_HOME_URL || tmp.substr(0, tmp.lastIndexOf("\/") + 1).replace("_examples/", "").replace("website/", ""); //这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）

    UEDITOR_CONFIG = {
        imagePath: URL + "../upload/uploadimages/", //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        ...
        filePath: URL + "../upload/uploadfiles/",  //附件文件夹保存路径
	    catchRemoteImageEnable:true,                                   //是否开启远程图片抓取
        catcherUrl:URL +"server/submit/php/getRemoteImage.php",             //处理远程图片抓取的地址
        localDomain:"wlniao.com",                                        //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地
