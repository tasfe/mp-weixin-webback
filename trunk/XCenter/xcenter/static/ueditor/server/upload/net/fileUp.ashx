...
public class fileUp : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        //上传配置
        String pathbase = cfgHelper.FrameworkRoot + "upload/uploadfiles/";                                      //保存路径
        string[] filetype = { ".exe", ".aspx", ".html", ".js", ".asp", ".ashx", ".cs", ".css" };    //文件允许格式
        ...
        try
        {
            ...
            title = System.IO.Path.GetFileNameWithoutExtension(uploadFile.FileName) + System.IO.Path.GetExtension(uploadFile.FileName);
            ...
            if (Array.IndexOf(filetype, currentType) >= 0)
            {
                state = "不支持的文件类型！";
            }
        ...
        //向浏览器返回数据json数据
        context.Response.Write("{'state':'" + state + "','url':'" + url + "','title':'" + title + "','fileType':'" + currentType + "'}");
    }
