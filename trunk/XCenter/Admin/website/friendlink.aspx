<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="friendlink.aspx.cs" Inherits="XCenterCMS.Web.Admin.news.friendlink" %>
<html>
    <head>
        <title><%=GetKeyValue("ManageSiteTitle")%>-<%=UIConfig("AdminTitleFriendLink")%></title><%=GetTemplate("admin/_tags")%>
        <style type="text/css">
            ...
            <div id="bgwrap">                
                <table id="main">
                    <tr>
                        ...
                        <td id="mainRight">
                        <h1><%=UIConfig("AdminTitleFriendLink")%></h1>
                                ...
                                <table style="width:100%;">
                                    <tr>
                                        <td style="width:100%;">
                                            ...
                                            <select id="FriendLinkType" name="FriendLinkType" class="mini-combobox" style="width:90px;" onvaluechanged="Select()" >
                                                <%=typelist%>
                                            </select>
                                        </td>
                                    </tr>
                        ...
                        <div id="datagrid" class="mini-datagrid" style="width:100%;height:360px;" allowResize="false" url="<%= WebRoot %>admin/website/friendlink.aspx?handle=GetList" onRowdblclick="edit" idField="Id" multiSelect="true">
                            <div property="columns">
                                ...
                                <div field="FriendLinkType" width="80" headerAlign="center" allowSort="false">分类</div>
                                <div field="FriendLinkTitle" width="380" headerAlign="center" allowSort="false">名称</div>
...
<script type="text/javascript">
    ...
    if (checkRight('FriendLinkManage')) {
        grid.load();
    ...
    function Select() {
        var type = mini.get("FriendLinkType").getValue();
        grid.load({ type: type });
    }
    function add() {
        if (checkRight('FriendLinkManage')) {
            mini.open({
                url: bootPATH + "../../../admin/website/friendlinkForm.aspx",
                title: "新增友情链接", width: 460, height: 398,
                maskOnLoad: false,
                onload: function () {
                    try {
    ...
    function edit() {
        if (checkRight('FriendLinkManage')) {
            ...
            if (row) {
                mini.open({
                    url: bootPATH + "../../../admin/website/friendlinkForm.aspx?action=edit&Id=" + row.Id,
                    title: "编辑友情链接", width: 460, height: 398,
                    maskOnLoad: false,
                    onload: function () {
                        var iframe = this.getIFrameEl();
            ...
            if (rows.length > 0) {
                if (confirm("确定删除选中记录？")) {
                    ...
                    $.ajax({
                        type: "POST",
                        dataType: 'json',
                        url: "<%= WebRoot %>admin/website/friendlink.aspx?handle=Del&ids=" + id,
