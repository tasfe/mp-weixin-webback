...
public class imageManager : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string path = context.Server.MapPath(cfgHelper.FrameworkRoot + "upload/uploadimages");                  //最好使用缩略图地址，否则当网速慢时可能会造成严重的延时
        ...
        if(action == "get")
        {
            ...
            if (info.Exists)
            {
                foreach (FileInfo fi in info.GetFiles())
                {
                    ...
                    {
                        str += cfgHelper.FrameworkRoot + "upload/uploadimages/" + fi.Name + "ue_separate_ue";
                    }
                }
            }
